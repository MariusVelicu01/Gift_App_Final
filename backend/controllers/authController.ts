...
export async function googleAuth(req: Request, res: Response) {
  try {
    ...
    const result = await loginOrInitGoogleUser(googleAccessToken.trim());
    ...
    const msg = String(error?.message || '');
    if (msg.includes('parolă') || msg.includes('invalid')) {
      return res.status(400).json({ message: msg });
    }
    return res.status(500).json({ message: 'Autentificarea Google a eșuat.' });
  }
}

export async function googleComplete(req: Request, res: Response) {
  try {
    const { googleAccessToken, firstName, lastName, birthDate, gender } = req.body;
